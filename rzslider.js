  ...
  .factory('RzSlider', ['$timeout', '$document', '$window', '$compile', 'RzSliderOptions', 'rzThrottle', function($timeout, $document, $window, $compile, RzSliderOptions, rzThrottle) {
    ...
    Slider.prototype = {
      ...
      translateFn: function(value, label, which, useCustomTr) {
        ...
        label.html(valStr);	
/*Create item for checking current position name*/		
		this.scope.active=valStr;
      ...
      updateTicksScale: function() {
        ...
        for (var i = 0; i < ticksCount; i++) {
		  ...
		  /*Custom colors for rainbow option*/
		 this.scope.rainbowBool=this.options.rainbow;
		...
		/*Customizing for rainbow option */
		 if (this.options.rainbow) {
		console.log(this.scope.rzSliderModel);
this.scope.bubleStyle=colors[this.scope.rzSliderModel];	
this.scope.minPointerStyle = {
            borderColor: this.scope.bubleStyle
          };
		  this.scope.ColorPointerTextStyle = {
            color: this.scope.bubleStyle
          };
console.log(this.scope.bubleStyle);	
			for (var y = 0; y < ticksCount-1; y++) {
		
		 this.scope.lines.push(colors[y]);
		 this.scope.lineWidth = 100/(ticksCount-1)+'%';
		 this.scope.lineColor=colors[y];
		 this.scope.linePosition='relative';
		 
		 }
			
		  }
		  else{
			  this.scope.lines=['#bcbec0'];
			  this.scope.lineWidth = '100%';
			 this.scope.lineColor='#bcbec0';
			 this.scope.linePosition='relative';
  ...
  $templateCache.put('rzSliderTpl.html',
    "<span class=rz-bar-wrapper><span ng-repeat=\"l in lines track by $index\" ng-style=\"{\'width\' : lineWidth, \'background\' : lines[$index], \'position\' : linePosition}\" class=rz-bar> </span></span><span class=rz-bar-wrapper><span class=\"rz-bar rz-selection\" ng-style=barStyle></span></span> <span class=\"rz-pointer rz-pointer-min\" ng-style=minPointerStyle ng-class=\"rainbowBool ? 'colorPointer':'simplePointer'\"></span> <span class=\"rz-pointer rz-pointer-max\" ng-style=maxPointerStyle ></span> <span class=\"rz-bubble rz-limit\"></span> <span class=\"rz-bubble rz-limit\"></span> <span class=rz-bubble></span> <span class=rz-bubble></span> <span class=rz-bubble></span><ul ng-show=showTicks class=rz-ticks><li ng-repeat=\"t in ticks track by $index\" class=rz-tick ng-class=\"{'rz-selected': t.selected}\" ng-style=t.style ng-attr-uib-tooltip=\"{{ t.tooltip }}\" ng-attr-tooltip-placement={{t.tooltipPlacement}} ng-attr-tooltip-append-to-body=\"{{ t.tooltip ? true : undefined}}\"><span ng-if=\"t.value != null\" ng-style=ColorPointerTextStyle ng-class=\"rainbowBool ?(t.value==active ? 'activeTip':'nonactiveTip'): (t.value==active ? 'activeTipHour':'nonactiveTip')\" class=rz-tick-value  ng-attr-uib-tooltip=\"{{ t.valueTooltip }}\" ng-attr-tooltip-placement={{t.valueTooltipPlacement}}>{{ t.value }}</span> <span ng-if=\"t.legend != null\" class=rz-tick-legend>{{ t.legend }}</span></li></ul>"
  );
